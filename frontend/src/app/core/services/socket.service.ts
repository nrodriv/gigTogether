...
export class SocketService implements OnDestroy {
  ...
  private _memberLeft$ = new Subject<{ groupId: string; userId: string; reason: 'left' | 'expelled' }>();
  ...
  connect(token: string): void {
    ...
    this.socket.on('memberLeft', (data: { groupId: string; userId: string; reason?: 'left' | 'expelled' }) =>
      this._memberLeft$.next({ ...data, reason: data.reason ?? 'left' }),
    );
